...
report_lines.extend([
    ...
    "|:---:|:---:|:---:|:---|:---|",
    f"| **Prompt v1** *(Bản nháp ban đầu)* | 14 / 20 | 70.0% | Thiếu grounding slide Day 02, câu hỏi còn chung chung | Cập nhật tri thức 29 slide bài giảng Day 02 |",
    f"| **Prompt v2 (Final)** | **{passed_count} / 20** | **{pass_rate}%** | Đã khắc phục triệt để các ca bẫy và lạc đề | Hoàn thiện cấu trúc JSON 4 trường, sẵn sàng nộp CP3 |",
    "",
    "---",
    "",
    ...
    "",
    "```text",
    f"Báo cáo kiểm thử AI Feature (Mốc CP3) — Nhóm Tứ Đại Thiên Vương:",
    f"- Tính năng: AI Học viên tò mò theo phương pháp Feynman (Bước 8 & 9).",
    f"- Bộ dữ liệu kiểm thử: 20 kịch bản thực tế trong Golden Set phủ 6 nhóm tình huống (Happy path, Sai kiến thức, Mơ hồ, Lạc đề, Phá vai, Edge cases) dựa trên 29 slide bài giảng Day 02.",
    f"- Kết quả đo lường: Đạt {passed_count}/20 ca kiểm thử (Tỷ lệ {pass_rate}%).",
    f"- Điểm nổi bật: AI chạy thực tế trên {MODEL_NAME} tuân thủ 100% cấu trúc JSON, giữ vững vai trò học sinh Socratic (không tự giảng bài thay người dùng) và xử lý xuất sắc các ca bẫy lạc đề / phá vai.",
    "```"
])
